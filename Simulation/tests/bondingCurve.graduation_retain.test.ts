...
type SwapRow = {
  ...
  tokens_to_distribute: string;
  tokens_saved: string;
  reserve_used: string;
  reserve_cumulative: string;
  effective_return_x: string;
...
function swapsToCsv(rows: SwapRow[]): string {
  const header =
    "swap_index,sol_in_SOL,price_per_token_SOL,retain_multiplier,tokens_to_distribute,tokens_saved,reserve_used,reserve_cumulative,effective_return_x,total_sol_raised,total_tokens_minted,tokens_out_estimate,tokens_out_actual";
  const body = rows
    .map(
      (r) =>
        [
          ...
          escapeCsv(r.tokens_to_distribute),
          escapeCsv(r.tokens_saved),
          escapeCsv(r.reserve_used),
          escapeCsv(r.reserve_cumulative),
          escapeCsv(r.effective_return_x),
...
async function runGraduationTest() {
  ...
  let swaps = 0;
  let reserveCumulative = new BN(0);
  const allSwaps: SwapRow[] = [];
  const lastSwaps: SwapRow[] = [];
  const graduationAmountSOL = DEFAULT_GRADUATION_AMOUNT_SOL;
  ...
  while (swaps < MAX_SWAPS) {
    ...
    const finalPriceSOL = getPricePerTokenInSol(poolId, graduationAmountSOL);
    ...
    reserveCumulative = reserveCumulative.add(tokens_saved).sub(reserve_used);

    const TOKEN_LAMPORTS = 1e9;
    const tokensDistWhole = tokens_to_distribute.div(new BN(TOKEN_LAMPORTS)).toNumber();
    ...
    const row: SwapRow = {
      ...
      reserve_cumulative: reserveCumulative.toString(),
    ...
    if (state.graduated) {
      ...
      for (const s of lastSwaps) {
        console.log(
          ...
          s.tokens_to_distribute,
          "| tokens_saved:",
          s.tokens_saved,
          "| reserve_used:",
          s.reserve_used,
          "| reserve_cumulative:",
          s.reserve_cumulative,
          "| effective_return_x:",
          s.effective_return_x,
          "| tokens_out_actual:",
          s.tokens_out_actual
        );
      }
      ...
      const csvPath = join(__dirname, "retain_graduation_swaps.csv");
