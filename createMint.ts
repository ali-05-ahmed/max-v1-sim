...
export const createMint = async () => {
  ...
  const { publicKey: platformId } = getPdaPlatformId(DEVNET_PROGRAM_ID.LAUNCHPAD_PROGRAM, owner.publicKey)
  ...
  const { execute, transactions, extInfo } = await raydium.launchpad.createLaunchpad({
    ...
    totalSellA: new BN(693_100_000_000_000),  // lauchpad mint sell amount, default: LaunchpadPoolInitParam.totalSellA
    // totalFundRaisingB: new BN(85_000_000_000),  // if mintB = SOL, means 85 SOL, default: LaunchpadPoolInitParam.totalFundRaisingB
    totalLockedAmount: new BN(100_000_000_000_000),  // total locked amount, default 0
    // cliffPeriod: new BN(0),  // unit: seconds, default 0
    unlockPeriod: new BN(1),  // unit: seconds, default 0
